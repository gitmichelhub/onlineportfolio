...
const ProjectsSection: React.FC = () => {
  ...
  return <section id="projects" className="min-h-screen py-20 bg-white">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid md:grid-cols-2 gap-8 mb-12">
          {projects.map((project, index) => <div key={index} className="glass rounded-2xl p-6 transition-all duration-300 hover:scale-105 hover:shadow-xl animate-fade-up group" style={{
          animationDelay: `${0.2 + index * 0.1}s`
              ...
              <div className="flex flex-wrap gap-2 mb-4">
                {project.technologies.map((tech, techIndex) => <span key={techIndex} className="px-3 py-1 bg-indigo-50 text-indigo-700 text-sm rounded-full">
                    {tech}
                  </span>)}
              </div>

              <div className="flex space-x-4">
                <a href={project.link} className="flex items-center space-x-2 text-indigo-600 hover:text-indigo-700 transition-colors">
                  <ExternalLink size={16} />
                  <span className="text-sm font-medium">View Live</span>
                </a>
                <a href={project.github} className="flex items-center space-x-2 text-slate-600 hover:text-slate-700 transition-colors">
                  <Github size={16} />
        ...
        <div className="text-center mb-8">
          <Collapsible open={isCVOpen} onOpenChange={setIsCVOpen}>
            <CollapsibleTrigger asChild>
              <button className="inline-flex items-center space-x-2 bg-indigo-600 text-white px-6 py-3 rounded-full font-medium transition-all duration-200 hover:scale-105 hover:bg-indigo-700" aria-expanded={isCVOpen} aria-controls="cv">
            ...
            <CollapsibleContent className="overflow-hidden transition-all duration-500 ease-in-out">
              <div id="cv" className="mt-12">
                ...
                <VerticalTimeline lineColor="#CBD5E1">
                  {timelineData.map((item, index) => <VerticalTimelineElement key={index} className="vertical-timeline-element--work animate-fade-up" style={{
